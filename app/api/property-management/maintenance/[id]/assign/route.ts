...
export async function POST(req: NextRequest, { params }: RouteParams) {
  try {
    ...
    const maintenanceRequest = await maintenanceService.assign(tenant.id, params.id, body)
    return NextResponse.json({ maintenanceRequest })
  } catch (error) {
    if (error instanceof ZodError) {
      return NextResponse.json({ error: error.flatten() }, { status: 400 })
    }
    console.error('[MaintenanceAssignAPI.POST] failed', { id: params.id, message: error instanceof Error ? error.message : 'unknown error' })
